...
open class PaddedTextField: UITextField {
    ...
    @IBInspectable open var selectedLineColor: UIColor = UIColor(red: 19/156.0, green: 141/156.0, blue: 117/156.0, alpha: 1.0) {
        didSet {
            self.floatTheLabel()
        }
    }
    ...
    @IBInspectable open var selectedPlaceHolderColor: UIColor =  #colorLiteral(red: 0.5764705882, green: 0.5843137155, blue: 0.5960784314, alpha: 1) {
        didSet {
            self.floatTheLabel()
        }
    }
    ...
    override open func textRect(forBounds bounds: CGRect) -> CGRect {
        ...
        if text == "" {
            return CGRect(x: 20, y: 1, width: bounds.size.width -  40, height: bounds.size.height)
        }
        return CGRect(x:  20, y:  12, width: bounds.size.width - 20, height: bounds.size.height)
    }
    // this variable is used for events in phone number field and based on that we sent the inset according to flag's width in phone number textfield.
...
fileprivate extension PaddedTextField {
    ...
    func addFloatingLabel() {
        ...
        labelPlaceholder?.textColor =  #colorLiteral(red: 0.5764705882, green: 0.5843137155, blue: 0.5960784314, alpha: 1)
        ...
        var topConstraint = NSLayoutConstraint.init(item: labelPlaceholder!, attribute: .top, relatedBy: .equal, toItem: self, attribute: .top, multiplier: 1, constant: 17)
        if UIDevice.current.userInterfaceIdiom == .pad {
             topConstraint = NSLayoutConstraint.init(item: labelPlaceholder!, attribute: .top, relatedBy: .equal, toItem: self, attribute: .top, multiplier: 1, constant: 17)
        }
            placeholderLabelHeight = NSLayoutConstraint.init(item: labelPlaceholder!, attribute: .height, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute, multiplier: 1, constant: 0)
        ...
        func upadteTextField(frame: CGRect) {
            self.frame = frame
            self.initialize()
        }
        
        // MARK: - Float UITextfield Placeholder Label
        func floatPlaceHolder(selected: Bool) {
            ...
            labelPlaceholder?.font = UIFont(name: (self.font?.fontName)!, size: 14)
            if UIDevice.current.userInterfaceIdiom == .pad {
